...
private [json] trait JsonMacroImplJvm extends MacroAdapter[Context] {
  ...
  def readJson[O: WeakTypeTag]: Tree = {
    ...
    val block = q"""{
      ...
      com.hypertino.binders.json.JsonBindersFactory.findFactory().withStreamParser[${weakTypeOf[O]}]($t.inputStream) { case ($d) =>
  ...
  def writeJson[O: WeakTypeTag](outputStream: Expr[OutputStream]): Tree = {
    ...
    val block = q"""{
      val $t = ${ctx.prefix.tree}
      com.hypertino.binders.json.JsonBindersFactory.findFactory().withStreamGenerator($outputStream) { case ($s) =>
